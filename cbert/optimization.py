...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, hvd=None, amp=False, accumulation_step=1,la_steps=1,la_alpha=0.5,freeze_bert=False,use_radam=False, head_lr_ratio=1.0):
  ...
  Args:
    ...
    la_steps: look ahead steps
    la_alpha: ratio for slow weight of look ahead 
    freeze_bert: whether to freeze bert variables
    use_radam: whether to use radam optimizer
    use_swa: whether to use swa, if true we should not let end learning rate become too small
  ...
  if la_steps > 1:
    tf.logging.info('## Using Look Ahead - Steps {}, Alpha {} ##'.format(la_steps,la_alpha))
    optimizer = Lookahead(optimizer,la_steps,la_alpha)

  if hvd is not None:
    from horovod.tensorflow.compression import Compression
    optimizer = hvd.DistributedOptimizer(optimizer, compression=Compression.fp16 if amp else Compression.none)
